...
class ProtoH(util.framework.FewShotNERModel):
    def __init__(self, word_encoder, dot=False, ignore_index=-1):
        ...
        self.alpha = nn.Parameter(torch.tensor(6.0, requires_grad=True))
    ...
    def forward(self, support, query):
        ...
        for i, sent_support_num in enumerate(support['sentence_num']):
            ...
            if self.cur_it < 300:
                ...
                min_dist = -dist[:, 1:].max(dim=-1, keepdim=True).values
                o_prob = min_dist / (
                    min_dist + torch.exp(self.alpha)
                )  # [num_of_query_tokens, 1]
                logits.append(
                    torch.cat(
                        [
                            o_prob.log() + dist[:, 1:].sum(dim=1, keepdim=True),
                            dist[:, 1:] + (1 - o_prob).log(),
                        ],
                        dim=1,
                    )
