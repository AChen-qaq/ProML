...
class TransferBERT(util.framework.FewShotNERModel):
    ...
    def __init__(self,word_encoder, dot=False, ignore_index=-1, train_classes=None, val_classes=None, test_classes=None):
        util.framework.FewShotNERModel.__init__(self, word_encoder, ignore_index=ignore_index)
        self.drop = nn.Dropout()
        self.dot = dot
        self.train_fc = nn.Linear(768, train_classes + 1)
        self.val_fc = nn.Linear(768, val_classes + 1)
        self.test_fc = nn.Linear(768, test_classes + 1)
        self.fc = self.train_fc
        self.flag = 0
    ...
    def __get_nearest_dist__(self, embedding, tag, mask, query, q_mask):
        ...
        for label in range(torch.max(tag)+1):
            nearest_dist.append(torch.topk(dist[:,tag==label], k=1, dim=1).values.mean(dim=1))
    ...
    def forward(self, support, query):
        ...
        for i, sent_support_num in enumerate(support['sentence_num']):
            ...
            q, m = query_emb[current_query_num:current_query_num+sent_query_num], query['text_mask'][current_query_num: current_query_num+sent_query_num]
            # Calculate nearest distance to single entity in each class in support set
            logits.append(self.fc(q[m==1].view(-1, q.size(-1))))
