...
class Pr(util.framework.FewShotNERModel):
    ...
    def __dist__(self, mean, vari, x, dim):
        epsilon = 1e-3
        # check(mean)
        # check(vari)
        # check(x)
        # exit()
        return (-torch.pow(x - mean, 2) / (vari + epsilon)-torch.log(vari + epsilon) ).sum(dim)
    ...
    def __batch_dist__(self, S, Q, q_mask):
        ...
        assert Q.size()[:2] == q_mask.size()
        Q = Q[q_mask==1].view(-1, Q.size(-1)) # [num_of_all_text_tokens, embed_dim]
        return self.__dist__(mean.unsqueeze(0), vari.unsqueeze(0), Q.unsqueeze(1), 2)
    ...
    def __get_proto__(self, embedding, tag, mask):
        ...
        assert tag.size(0) == embedding.size(0)
        for label in range(torch.max(tag)+1):
            mean.append(torch.mean(embedding[tag==label], 0))
            vari.append(torch.var(embedding[tag==label], 0, unbiased=True))
        mean = torch.stack(mean)
        vari = torch.stack(vari) #.detach()
        return mean, vari
    ...
    def forward(self, support, query):
        ...
        '''
        # print([(x, type(y), len(y)) for x,y in support.items()])
        # print([(x, type(y), len(y)) for x,y in query.items()])
        # exit()
